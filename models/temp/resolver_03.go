...
func (r *mutationResolver) ActivityApproval(ctx context.Context, domain string, workflowID string, runID *string, activityID *string) (*bool, error) {
	state := "APPROVED"
	var result bool

	err := workflowClient.CompleteActivityByID(context.Background(), domain, workflowID, *runID, *activityID, state, nil)
	if err != nil {

		log.Println("ОШИБКА! Не удалось подтвердить активность. " + err.Error())
		result = false
		return nil, err
	}
...
func (r *mutationResolver) ActivityCancel(ctx context.Context, domain string, workflowID string, runID *string, activityID *string) (*bool, error) {
	state := "CANCELLED"
	var result bool
	//добавить детали - причину отмены активности и кто ее отменил
	err := workflowClient.CompleteActivityByID(context.Background(), domain, workflowID, *runID, *activityID, nil, cadence.NewCustomError(state, "USER1"))
	if err != nil {

		log.Println("ОШИБКА! Не удалось отменить активность. " + err.Error())
		result = false
		return nil, err
	}
...
func (r *mutationResolver) ActivityTerminate(ctx context.Context, domain string, workflowID string, runID *string, activityID *string) (*bool, error) {
	panic("not implemented")
}
